...
exports.getHasilUSM = (req, res, next) => {
    ...
    if (typeof req.session.username === 'undefined') {
      ...
      resClient.then(dataClient => {
        ...
        resUser.then(dataUser => {
          ...
          inputHasilUSMData(username, token, '1', res, req)
        })
      }).catch(err => console.log(err))
    } else {  
      if (req.params.page) {
        inputHasilUSMData(req.session.username, req.session.token, req.params.page, res, req)
      } else {
        inputHasilUSMData(req.session.username, req.session.token, '1', res, req)
      }
    }
    
};

inputHasilUSMData = (username, token, page, res, req) => {
  const resHasilUSM = axios.getAllHasilUSM(username, token, page);  
  resHasilUSM.then(dataUSM => {
    let arr = [];
