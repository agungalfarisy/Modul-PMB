...
exports.getFileUSM = (req, res, next) => {
    ...
    if (typeof req.session.username === 'undefined') {
      ...
      resClient.then(dataClient => {
        ...
        resUser.then(dataUser => {
          ...
          inputFileUSMData(username, token, '1', res, req)
        })
      }).catch(err => console.log(err))
    } else {  
      if (req.params.page) {
        inputFileUSMData(req.session.username, req.session.token, req.params.page, res, req)
      } else {
        inputFileUSMData(req.session.username, req.session.token, '1', res, req)
      }
    }
    
};

inputFileUSMData = (username, token, page, res, req) => {
