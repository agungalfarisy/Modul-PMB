...
exports.getInfoPeserta = (req, res, next) => {
    ...
    if (typeof req.session.username === 'undefined') {
      ...
      resClient.then(dataClient => {
        ...
        resUser.then(dataUser => {
          ...
          inputInfoPesertaData(username, token, '1', res, req)
        })
      }).catch(err => console.log(err))
    } else {  
      if (req.params.page) {
        inputInfoPesertaData(req.session.username, req.session.token, req.params.page, res, req)
      } else {
        inputInfoPesertaData(req.session.username, req.session.token, '1', res, req)
      }
    }
    
};

inputInfoPesertaData = (username, token, page, res, req) => {
  const resUSM = axios.getAllInfoPeserta(username, token, page);  
  resUSM.then(dataUSM => {
    let arr = [];
